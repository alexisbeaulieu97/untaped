...
def test_list_jobs_defaults_to_kind_job() -> None:
    repo = _FakeJobRepo()
    list(ListJobs(repo)())  # type: ignore[arg-type]
...
def test_list_jobs_applies_newest_first_default() -> None:
    ...
    repo = _FakeJobRepo()
    list(ListJobs(repo)(kind="workflow_job"))  # type: ignore[arg-type]
...
def test_list_jobs_preserves_caller_order_by() -> None:
    ...
    repo = _FakeJobRepo()
    list(ListJobs(repo)(params={"order_by": "started", "status": "running"}))  # type: ignore[arg-type]
...
def test_list_jobs_forwards_kind_and_limit() -> None:
    ...
    out = list(
        ListJobs(repo)(  # type: ignore[arg-type]
            kind="project_update",
...
def test_list_jobs_handles_none_params() -> None:
    ...
    repo = _FakeJobRepo()
    list(ListJobs(repo)(params=None))  # type: ignore[arg-type]
    assert repo.list_calls[0]["params"] == {"order_by": "-id"}
