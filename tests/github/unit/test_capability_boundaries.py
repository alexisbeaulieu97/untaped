"""Kernel-only import boundaries for the github built-in (Wave 2 slice 1, §6(b)).

The github capability MUST NOT import another capability's
implementation modules (``untaped.capabilities.<name>.*`` for
``<name> != "github"``), and no ``untaped-github`` standalone
...
SRC_ROOT = REPO_ROOT / "src" / "untaped" / "capabilities" / "github"
TESTS_ROOT = REPO_ROOT / "tests" / "github"
...
def test_github_imports_no_sibling_capability() -> None:
    ...
    for py_file in sorted(SRC_ROOT.rglob("*.py")):
        for target in _capability_targets(py_file):
            ...
            if rest == "capabilities" or rest.startswith("capabilities."):
                parts = rest.split(".")
                if len(parts) >= 2 and parts[1] not in ("github", "registry"):
                    violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {target}")
    assert not violations, (
        "github capability must not import sibling capabilities (§6(b)):\n  "
        + "\n  ".join(violations)
    )
        ...
        for py_file in sorted(root.rglob("*.py")):
            ...
            for imp in _runtime_imports(tree):
                ...
                if isinstance(imp, ast.Import):
                    ...
                    names = [imp.module]
                for name in names:
                    if name == "untaped_github" or name.startswith("untaped_github."):
                        violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {name}")
    assert not violations, "no untaped-github standalone imports may survive:\n  " + "\n  ".join(
        violations
    )


def test_no_standalone_composition_imports_in_github() -> None:
    violations: list[str] = []
    ...
    assert not violations, (
        "github tree must not use retired ToolSpec composition (§9 gate 2):\n  "
        + "\n  ".join(violations)
    )
