...
from orchestration.builders import DECISION_ID, STORE_ID, TASK_ID
        ...
        completeness = Completeness(
            (
                IncompleteStore(
                    ...
                    "missing",
                    __import__(
                        "untaped.capabilities.orchestration.domain.diagnostics",
                        fromlist=["Diagnostic"],
                    ).Diagnostic(
                        code="ORC005",
                        severity="warning",
...
def test_raw_inspect_runs_inside_the_selected_local_lease_without_loading_children() -> None:
    from untaped.capabilities.orchestration.application import query_models
...
def test_invalid_canonical_store_makes_partial_reads_incomplete_and_readiness_fail_closed() -> None:
    ...
    diagnostic = __import__(
        "untaped.capabilities.orchestration.domain.diagnostics", fromlist=["Diagnostic"]
    ).Diagnostic(
        code="ORC001",
        severity="error",
