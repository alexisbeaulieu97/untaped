...
from orchestration.unit.application.test_decision_lifecycle import (
    create_decision,
)
from orchestration.unit.application.test_decision_lifecycle import (
    service as decision_service,
)
from orchestration.unit.application.test_relations import _state as relation_state
from orchestration.unit.application.test_task_transition import create, state
from untaped.capabilities.orchestration.application.curation import (
    AcknowledgeRequest,
    CurationService,
    SnoozeRequest,
)
...
from untaped.capabilities.orchestration.application.tasks import (
    CloseTaskRequest,
    MoveTaskRequest,
    TaskLifecycleConflict,
    TransitionTaskRequest,
)
from untaped.capabilities.orchestration.domain.evidence import EvidenceReference, EvidenceRelation
from untaped.capabilities.orchestration.domain.ids import DecisionId, StoreId, TaskId
from untaped.capabilities.orchestration.domain.models import LinkRelation, TaskOutcome, TaskStage
from untaped.capabilities.orchestration.domain.ordering import PlacementAnchor, PlacementAnchorKind
...
def test_force_current_relations_and_curation_skip_only_owner_revision(tmp_path) -> None:
    ...
    linked = ChangeLink(executor, repository).add(
        scope,
        __import__(
            "untaped.capabilities.orchestration.application.items", fromlist=["LinkRequest"]
        ).LinkRequest(
            task.record.metadata.id,
    ...
    evidenced = ChangeEvidence(executor, repository).add(
        scope,
        __import__(
            "untaped.capabilities.orchestration.application.items", fromlist=["EvidenceRequest"]
        ).EvidenceRequest(
            task.record.metadata.id,
    ...
    curation = CurationService(
        ...
        repository,
        __import__(
            "orchestration.unit.application.test_task_transition", fromlist=["Clock"]
        ).Clock(),
        scope,
    )
