...
PACKAGE_NAME = "untaped.capabilities.orchestration"
PACKAGE_ROOT = Path(__file__).parents[3] / "src" / "untaped" / "capabilities" / "orchestration"
...
def _is_sdk_import(reference: ImportReference) -> bool:
    if reference.module == PACKAGE_NAME or reference.module.startswith(f"{PACKAGE_NAME}."):
        return False
...
def _is_allowed_sdk_import(reference: ImportReference, source_layer: str) -> bool:
    if source_layer == "cli" and reference.module == "untaped.api":
        return True
    # The capability root is composition surface (SPEC wiring), not a DDD
    # layer: it may import the stable provider surface like cli/ may use untaped.api.
    return source_layer == "root" and reference.module == "untaped.capability_api"
...
def test_scanner_rejects_every_forbidden_dependency(tmp_path: Path) -> None:
    package_root = tmp_path / "untaped.capabilities.orchestration"
    forbidden_imports = {
        "domain/rules.py": (
            "untaped.capabilities.orchestration.application",
            "untaped.capabilities.orchestration.infrastructure",
            "untaped.capabilities.orchestration.cli",
            "untaped.api",
        ),
    ...
    from_import.write_text(
        "from untaped.capabilities.orchestration import application\n",
        encoding="utf-8",
    )
    expected_count += 1
...
def test_scanner_restricts_infrastructure_to_application_ports(tmp_path: Path) -> None:
    ...
    repository.write_text(
        "import untaped.capabilities.orchestration.application.use_case\n",
        encoding="utf-8",
    )
...
def test_scanner_allows_inward_dependencies_and_cli_composition(tmp_path: Path) -> None:
    package_root = tmp_path / "untaped.capabilities.orchestration"
    allowed_imports = {
        "application/use_case.py": "untaped.capabilities.orchestration.domain",
        "infrastructure/repository.py": "untaped.capabilities.orchestration.application.ports",
        "cli/commands.py": "untaped.capabilities.orchestration.infrastructure",
