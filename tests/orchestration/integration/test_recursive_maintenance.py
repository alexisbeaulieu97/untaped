...
from orchestration.builders import (
    CHILD_STORE_ID,
    DECISION_ID,
    STORE_ID,
    TASK_ID,
...
from untaped.capabilities.orchestration import build_app
from untaped.capabilities.orchestration.application.federation import FederationService
...
from untaped.capabilities.orchestration.application.scaffold import AGENTS_BYTES
from untaped.capabilities.orchestration.cli.context import CliContext
from untaped.capabilities.orchestration.domain.models import ArchivedTask, TaskOutcome, TaskStage
from untaped.capabilities.orchestration.infrastructure.codec import CanonicalStoreFormatter
...
from untaped.capabilities.orchestration.infrastructure.views import MarkdownViewRenderer, ViewError
...
def test_cli_local_check_fmt_and_render_accept_cross_store_navigation_with_archive(
    ...
    def invoke(*tokens: str) -> tuple[int, dict[str, object]]:
        with pytest.raises(SystemExit) as raised:
            app([*tokens, "--store", str(parent), "--format", "json"])
...
def test_local_maintenance_never_loads_or_locks_registered_children(
    ...
    context.maintenance().check(RecursiveCheckRequest(location, local=True))
    context.maintenance().fmt_check(RecursiveFormatRequest(location, local=True))
    context.maintenance().fmt_write(
        RecursiveFormatRequest(location, local=True), expected_store_revision=revision
    )
    context.maintenance().render_check(location)
    context.maintenance().render_write(location)
