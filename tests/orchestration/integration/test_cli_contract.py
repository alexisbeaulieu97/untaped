"""End-to-end CLI tests for the orchestration capability app.

Ports the standalone ``test_cli_contract.py`` to the unified composition
(spec §9 gate 2): the retired ``ToolSpec``/``run_tool`` ``main()`` entry
is replaced by direct :func:`run_cyclopts_app` invocation of the
capability app, so no ``sys.argv`` staging is needed. The standalone
``--version`` assertion is dropped: ``--version`` reports the unified
distribution version (§10 intentionally-replace). All envelopes, exit
codes, and byte-level assertions are otherwise unchanged.
"""
...
import untaped.capabilities.orchestration.application.curation as curation_module
from orchestration.builders import CHILD_STORE_ID, DECISION_ID, STORE_ID, TASK_ID, task_bytes
from orchestration.cli_fixtures import initialized_repository
from untaped.capabilities.orchestration.application.maintenance import (
    RecursiveCheckResult,
    RecursiveFormatResult,
)
from untaped.capabilities.orchestration.application.results import StoreLocation
from untaped.capabilities.orchestration.cli import app, maintenance_commands
from untaped.capabilities.orchestration.domain.diagnostics import Diagnostic
...
def _exit_code(tokens: list[str]) -> int:
    """Run the capability app on ``tokens``; cyclopts exits rather than returning."""
    try:
        run_cyclopts_app(app, tokens)
...
def test_id_new_json_uses_stable_envelope(capsys) -> None:
    assert _exit_code(["id", "new", "decision", "--format", "json"]) == 0
...
def _invoke(capsys, *tokens: str) -> tuple[dict[str, object], str]:
    assert _exit_code(list(tokens)) == 0
    captured = capsys.readouterr()
...
def test_cli_list_is_header_bounded_and_show_reads_body_inside_lock(
    ...
    listed, _ = _invoke(capsys, "list", "--store", str(root), "--format", "json")
...
def test_real_store_read_mutation_and_maintenance_families(tmp_path, capsys) -> None:
    root = initialized_repository(tmp_path)
...
def test_check_invalid_store_keeps_results_and_exits_one(tmp_path, capsys) -> None:
    root = initialized_repository(tmp_path)
    root.joinpath("views", "decisions.md").unlink()
    assert _exit_code(["check", "--store", str(root), "--format", "json"]) == 1
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
...
def test_check_missing_child_warns_unless_children_are_required(tmp_path, capsys) -> None:
    root = initialized_repository(tmp_path)
...
''',
    ...
    tokens = ["check", "--require-children", "--store", str(root), "--format", "json"]
    assert _exit_code(tokens) == 3
...
def test_read_only_maintenance_orc007_result_exits_four_through_actual_cli(
    ...
    assert _exit_code(tokens) == 4
    captured = capsys.readouterr()
    assert "ORC007" in captured.out + captured.err
    assert "Traceback" not in captured.out + captured.err


def test_cli_binary_recovery_and_json_base64_matrix(tmp_path, capfdbinary) -> None:
    root = initialized_repository(tmp_path)
    ...
    assert _exit_code(["show", DECISION_ID, "--raw", "--store", str(root)]) == 0
    ...
    assert _exit_code(["show", DECISION_ID, "--raw", "--store", str(root), "--format", "json"]) == 0
    ...
    assert (
        _exit_code(["show", DECISION_ID, "--raw", "--store", str(root), "--format", "table"]) == 2
    )
