"""Capability-construction checks for the jira capability (Wave 2 slice 2).

Replaces the standalone ``test_tool_entrypoint.py`` (``ToolSpec`` +
``run_tool`` + ``untaped-jira`` console script): jira now ships a
static ``SPEC: CapabilitySpec`` plus a nullary ``build_app()`` factory,
mounted as built-in ``untaped jira ...`` under the unified root.
...
from untaped.capabilities.jira import SPEC, build_app
...
def test_spec_is_jira_capability() -> None:
    assert isinstance(SPEC, CapabilitySpec)
    assert SPEC.name == "jira"
    assert SPEC.config_section == "jira"
    assert SPEC.profile_model is JiraSettings
    ...
    assert skill.description == "Use the untaped-jira CLI."
...
def test_no_standalone_console_script() -> None:
    ...
    assert "untaped-jira" not in scripts


def test_jira_mounts_under_root(_isolate: Path) -> None:
    root = _root()
    result = CliInvoker().invoke(root.meta, ["jira", "--help"])
    assert result.exit_code == 0, result.output
    for cmd in ("me", "issue", "project", "board", "sprint"):
        assert cmd in result.stdout


def test_top_level_help_lists_jira(_isolate: Path) -> None:
    root = _root()
...
def test_config_list_includes_profile_fields(_isolate: Path) -> None:
    ...
    assert "jira.token" in keys
    assert "jira.assigned_jql" in keys
...
def test_profile_field_resolves_from_profile_scope(_isolate: Path) -> None:
    ...
    get_settings.cache_clear()
    root = _root()
    result = CliInvoker().invoke(root.meta, ["config", "get", "jira.base_url"])
    assert result.exit_code == 0, result.output
    assert result.stdout.strip() == "https://jira.example.com"
...
def test_help_renders_unified_prefix(_isolate: Path) -> None:
    ...
    result = CliInvoker().invoke(root.meta, ["jira", "--help"])
    assert result.exit_code == 0, result.output
    assert "untaped-jira" not in result.output
