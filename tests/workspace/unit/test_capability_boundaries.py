"""Kernel-only import boundaries for the workspace built-in (Wave 1.5 §6(b)).

The workspace capability MUST NOT import another capability's
implementation modules (``untaped.capabilities.<name>.*`` for
``<name> != "workspace"``), and no ``untaped-workspace`` standalone
...
SRC_ROOT = REPO_ROOT / "src" / "untaped" / "capabilities" / "workspace"
TESTS_ROOT = REPO_ROOT / "tests" / "workspace"
...
def test_workspace_imports_no_sibling_capability() -> None:
    ...
    for py_file in sorted(SRC_ROOT.rglob("*.py")):
        for target in _capability_targets(py_file):
            ...
            if rest == "capabilities" or rest.startswith("capabilities."):
                parts = rest.split(".")
                if len(parts) >= 2 and parts[1] not in ("workspace", "registry"):
                    violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {target}")
    assert not violations, (
        "workspace capability must not import sibling capabilities (§6(b)):\n  "
        + "\n  ".join(violations)
    )
        ...
        for py_file in sorted(root.rglob("*.py")):
            ...
            for imp in _runtime_imports(tree):
                ...
                if isinstance(imp, ast.Import):
                    ...
                    names = [imp.module]
                for name in names:
                    if name == "untaped_workspace" or name.startswith("untaped_workspace."):
                        violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {name}")
    assert not violations, "no untaped-workspace standalone imports may survive:\n  " + "\n  ".join(
        violations
    )


def test_no_standalone_composition_imports_in_workspace() -> None:
    violations: list[str] = []
    ...
    assert not violations, (
        "workspace tree must not use retired ToolSpec composition (§9 gate 2):\n  "
        + "\n  ".join(violations)
    )
