"""Kernel-only import boundaries for the awx built-in (Wave 2 slice 3, §6(b)).

The awx capability MUST NOT import another capability's
implementation modules (``untaped.capabilities.<name>.*`` for
``<name> != "awx"``), and no ``untaped-awx`` standalone
...
SRC_ROOT = REPO_ROOT / "src" / "untaped" / "capabilities" / "awx"
TESTS_ROOT = REPO_ROOT / "tests" / "awx"
...
def test_awx_imports_no_sibling_capability() -> None:
    ...
    for py_file in sorted(SRC_ROOT.rglob("*.py")):
        for target in _capability_targets(py_file):
            ...
            if rest == "capabilities" or rest.startswith("capabilities."):
                parts = rest.split(".")
                if len(parts) >= 2 and parts[1] not in ("awx", "registry"):
                    violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {target}")
    assert not violations, (
        "awx capability must not import sibling capabilities (§6(b)):\n  " + "\n  ".join(violations)
    )
        ...
        for py_file in sorted(root.rglob("*.py")):
            ...
            for imp in _runtime_imports(tree):
                ...
                if isinstance(imp, ast.Import):
                    ...
                    names = [imp.module]
                for name in names:
                    if name == "untaped_awx" or name.startswith("untaped_awx."):
                        violations.append(f"{py_file.relative_to(REPO_ROOT)} imports {name}")
    assert not violations, "no untaped-awx standalone imports may survive:\n  " + "\n  ".join(
        violations
    )


def test_no_standalone_composition_imports_in_awx() -> None:
    violations: list[str] = []
    ...
    assert not violations, (
        "awx tree must not use retired ToolSpec composition (§9 gate 2):\n  "
        + "\n  ".join(violations)
    )
