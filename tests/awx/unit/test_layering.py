...
def test_awx_application_does_not_read_infrastructure_only_spec_fields() -> None:
    """``untaped.capabilities.awx/application/`` must not read fields that exist only on
    ...
    assert not violations, (
        f"untaped.capabilities.awx/application must not read AwxResourceSpec-only fields "
...
def _discover_package_roots() -> list[tuple[str, Path]]:
    ...
    return [("untaped.capabilities.awx", SRC_DIR)]
...
def _discover_infrastructure_dirs() -> list[tuple[str, Path]]:
    ...
    return [("untaped.capabilities.awx", SRC_DIR / "infrastructure")]
