...
`ACTION_BUILDERS` dict in `cli/factory.py`. Unknown names fall
...
from untaped.capabilities.awx.cli.factory import ACTION_BUILDERS
...
def test_every_actionspec_name_has_a_builder() -> None:
    ...
    assert not missing, (
        ...
        "builder + registry entry in cli/factory.py."
    )
