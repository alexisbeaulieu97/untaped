# AGENTS.md - `untaped workspace` capability

Git-workspace companion to the unified suite-wide guides:
[`docs/configuration.md`](../configuration.md),
[`docs/tool-conventions.md`](../tool-conventions.md), and the
[capabilities composition contract](../capabilities-spec.md).
This file keeps only `untaped workspace` capability-local rules, contracts, and gotchas.

## Mission

`untaped workspace` is a built-in capability of the unified `untaped` CLI, invoked as
...
1. **Keep `AGENTS.md` and the packaged skill up to date.** Architecture changes,
   ...
   `src/untaped/capabilities/workspace/skills/untaped-workspace/SKILL.md`.
2. **Keep the workspace `CapabilitySpec` particulars stable.** There is no
   standalone console script and no `ToolSpec`: `SPEC` declares
   `name="workspace"`, `config_section="workspace"`,
   `profile_model=WorkspaceSettings`, `state_model=WorkspaceState`, and one
   packaged skill named `untaped-workspace`, with a nullary `build_app()`
   factory. Importing `untaped.capabilities.workspace` never imports the
   command tree (`build_app` imports `cli` on demand at mount time).
...
```text
src/untaped/capabilities/workspace/
├── __init__.py           # SPEC: CapabilitySpec + nullary build_app() factory
...
`SPEC` is a static `CapabilitySpec(name="workspace", ...)` registered as a
built-in in `untaped.bootstrap` and mounted as `untaped workspace ...`. It
declares `WorkspaceSettings` as the `workspace` profile settings section,
`WorkspaceState` as the top-level `workspace` app-state section, and the
packaged `untaped-workspace` agent skill. Keep that static skill asset
...
profile/state split are pinned by `tests/workspace/unit/test_capability_entrypoint.py`.
...
live in `tests/workspace/conftest.py`. Sibling unit tests import them via
`from workspace.conftest import StubGit, StubRegistry, StubFilesystem,
StubManifests, empty_manifest` — `tests/workspace` ships an `__init__.py`
making `workspace` a top-level test package, so no `pythonpath` entry is
needed. New shared scaffolding for this package belongs in the same
module.
...
- [`usage.md`](./usage.md) - user-facing manifest,
