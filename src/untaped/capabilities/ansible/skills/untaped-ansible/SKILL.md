...
- Live reads that fail for one repo (deleted repo or tag, lost access) become graph warnings and leave that node unexpanded; auth (401) and rate-limit (429) failures still abort.
...
- Aliases are applied when sources are refreshed; after `alias add`/`alias remove`, run `untaped ansible source refresh NAME` for cached graphs to pick up the change.
- Row-style commands (`alias list`, `source list`, `source show`, `source status`) accept `--format pipe` for typed NDJSON: `untaped ansible source list --format pipe` emits one `{"untaped":"1","kind":"ansible.source","record":{...}}` line per row (kinds: `ansible.alias`, `ansible.source`, `ansible.source_status`). `graph` does not support `--format pipe`.
...
- The SQLite index is a cache with an enforced schema version and no migrations. On a version mismatch (older cache, or one written by a newer untaped), commands fail with an actionable error naming the exact index file to delete and the `untaped ansible source refresh NAME` command to run afterwards.
