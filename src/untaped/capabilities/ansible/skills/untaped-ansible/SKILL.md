...
- `graph --contains OWNER/REPO` (repeatable) searches downstream from one `TARGET` or many roots read with `--stdin` (bare `owner/repo@ref` lines, or pipe records with `scm_url`/`repo_url`/`repo`/`full_name` and `effective_scm_ref`/`ref`, e.g. `awx job-templates list --with-scm --format pipe`). It prints one `ansible.dependency_match` row per matched node (`root_repo`, `root_ref`, `repo`, `declared_ref` verbatim, `declared_in`, shortest `path`) as table/json/pipe, and nothing for roots that never reach it. Downstream only; honors `--depth`, `--source` and the refresh flags; `--stdin` requires `--contains`.
...
- Row-style commands (`alias list`, `source list`, `source get`, `source status`) accept `--format pipe` for typed NDJSON: `untaped ansible source list --format pipe` emits one `{"untaped":"1","kind":"ansible.source","record":{...}}` line per row (kinds: `ansible.alias`, `ansible.source`, `ansible.source_status`). `graph` supports `--format pipe` only with `--contains`.
