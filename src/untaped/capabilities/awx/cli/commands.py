...
from untaped.capabilities.awx.cli._selection import is_pipe_line
...
def _job_targets(
    ...
    if stdin and not job_ids:
        ...
        if not is_pipe_line(lines[0][1]):
            return [text for _, text in lines], {}
        ids: list[str] = []
        kinds: dict[str, str] = {}
        for lineno, text in lines:
            record = parse_envelope_line(lineno, text).record
        ...
        return ids, kinds
    return read_identifiers(list(job_ids or []), stdin=stdin, id_field="id"), {}
