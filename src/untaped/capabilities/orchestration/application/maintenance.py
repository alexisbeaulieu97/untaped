...
from untaped.capabilities.orchestration.application.import_operations import (
...
from untaped.capabilities.orchestration.application.mutations import validate_selected_local
from untaped.capabilities.orchestration.application.ports import (
    CanonicalFormatter,
    FileReplacement,
    LockManager,
    StoreLocation,
...
from untaped.capabilities.orchestration.application.repair_operations import (
...
from untaped.capabilities.orchestration.application.scaffold import (
    ShapeInspection,
    inspect_store_shape,
)
from untaped.capabilities.orchestration.application.validation import validate_snapshot
from untaped.capabilities.orchestration.application.view_management import (
    apply_views,
    view_comparisons,
)
...
# Bare `except A, B:` clauses are Python 3.14-only syntax and ruff format
# strips literal tuple parens under the 3.14 target, so the floor-safe
# spelling is a named alias (also keeps the C901 gate flat).
_VIEW_COMPARISON_ERRORS = (OSError, ValueError)
        ...
        with self._locks.acquire((location,), timeout=self._lock_timeout):
            ...
            if write:
                ...
                try:
                    ...
                    raise
                except _VIEW_COMPARISON_ERRORS:
                    views_current = False
            result_comparisons = (
