...
- Prune safety is local-only. `sync --prune` skips, and `remove --prune` / `forget --prune` refuse, when a clone has dirty/untracked/staged work, stash entries, or commits/local tags not reachable from local remote-tracking refs. The prune check does not fetch; stale remote-tracking refs are trusted as the offline safety boundary. `remove --prune` and `forget --prune` use the SDK batch preview/confirmation contract; a declined prompt exits cleanly without mutation. `forget --prune` checks declared repo paths and immediate child `.git` directories before deleting the workspace directory.
